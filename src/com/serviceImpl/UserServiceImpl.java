...
import com.constraint.UserConstraints;
...
public class UserServiceImpl implements UserServices{
	...
	//sign up method
	public boolean signUp(String name, String password, String question, String answer) throws DuplicateUserException, SQLException {
		boolean ret = false;
		
		//check username validity, password constraints, check where they are blanck or not
		if(UserConstraints.isValidUserName(name) && UserConstraints.isValidPassword(password) && (!question.isBlank()) && (!answer.isBlank())) {
			...
			if(user != null) {
				ret = false;
				throw (new DuplicateUserException("You are allready registered!"));
	...
	public boolean updatePassword(String name, String password, String confirmPassword) throws SQLException {
		...
		if(password.equals(confirmPassword)) {
			...
			if(UserConstraints.isValidPassword(password)) {
